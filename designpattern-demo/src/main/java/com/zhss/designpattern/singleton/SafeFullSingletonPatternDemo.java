...
public class SafeFullSingletonPatternDemo {
    ...
    public static class Singleton {
        ...
        public static Singleton getInstance() {
            ...
            // 3 线程2此时判断发现instance == null，也会进去
            if(instance == null) {
                // 2 线程1就会进来，此时线程1停止，切换到线程2
                // 4 线程2也会进来，此时切换到线程1

                // 5 线程1，发现这里需要加锁， 在这里加锁，获取到了这个锁
                // 7 线程2过来，线程2发现说，我也想要在这里加锁，发现说这个锁被人加了，线程2挂起等待别人释放锁
                // 11 此时切换回线程2，线程2发现锁被释放，然后在这里加锁
                synchronized(SafeFullSingletonPatternDemo.class) {
                    // 6 线程1就进来了，此时切换到线程2
                    // 8 切换回线程1，线程1此时在这里，再次判断，instance == null
                    // 12 线程2就进来了，double check，如果这里没有instance == null的判断，那么线程2就会再次创建
                    // 一个实例
                    // 13 但是这里是双重检查，线程2又判断了一下，instance == null？否，不是null
                    if(instance == null) {
                        // 9 线程1就会进来，创建一个实例
            ...
            // 10 这边出来以后，线程1就释放锁了
            // 14 线程2跳出来，直接获取一个instance返回了，这个instance就是之前线程1创建的实例
            return instance;
        }
